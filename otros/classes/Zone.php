...
Class Zone extends DBConnection {
	...
	public function save_zone(){
		...
		if(empty($id)){
			...
			if($chk > 0){
				...
				if($qry){
					$this->settings->set_flashdata('Guardado','Zona guardada con éxito.');
					return 1;
				}else{
			...
			if($chk > 0){
				...
				if($qry){
					$this->settings->set_flashdata('Guardado','Zona actualizada con éxito.');
					return 1;
				}else{
					return "INSERT INTO barangay_list set {$data}";
				}
	...
	public function delete_zone(){
		...
		if($qry){
			$this->settings->set_flashdata('Guardado','Zona eliminada con éxito.');
			return 1;
		}else{
			return false;
		}
